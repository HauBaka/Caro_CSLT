	...
	while (true) {
		if (_kbhit()) {
			...
			if (n == 13) { 
				...
				switch (c) {
				case 'w': loc_y = max(0, loc_y - 1); break;
				case 'a': loc_x = max(0, loc_x - 1); break;
				case 's': loc_y = min(game.board_heigh - 1, loc_y + 1); break;
				case 'd': loc_x = min(game.board_width - 1, loc_x + 1); break;
				}
